...
static void ti_sn_bridge_set_dsi_dp_rate(struct ti_sn_bridge *pdata)
{
	...
	DRM_INFO("mode->clock=%d clk_freq_mhz = %d\n",mode->clock,clk_freq_mhz);

	regmap_write(pdata->regmap, SN_DSIA_CLK_FREQ_REG, val);

	/* set DP data rate */
	dp_rate_mhz = ((bit_rate_mhz / pdata->dsi->lanes * 2 ) * DP_CLK_FUDGE_NUM) /
							DP_CLK_FUDGE_DEN;
	for (i = 0; i < ARRAY_SIZE(ti_sn_bridge_dp_rate_lut) - 1; i++)
		if (ti_sn_bridge_dp_rate_lut[i] > dp_rate_mhz)
			break;

	DRM_INFO("DP DATA RATE calucalted = %d\n",dp_rate_mhz);
...
static void ti_sn_bridge_enable(struct drm_bridge *bridge)
{
	...
	/* DSI_B lane config TODO - now always 0*/
        val = CHB_DSI_LANES(0);
        regmap_update_bits(pdata->regmap, SN_DSI_LANES_REG,
                           CHB_DSI_LANES_MASK, val);

	/* DP lane config */
	val = DP_NUM_LANES(pdata->dsi->lanes - 2);
	regmap_update_bits(pdata->regmap, SN_SSC_CONFIG_REG, DP_NUM_LANES_MASK,
			   val);

	/* DP lane SSC spread config */
        val = DP_SSC_SPREAD(0);
        regmap_update_bits(pdata->regmap, SN_SSC_CONFIG_REG, DP_SSC_SPREAD_MASK,
                           val);
	 ...
	 */
	DRM_INFO("ASSR Display Authentication method \n");
			      ...
			      const struct i2c_device_id *id)
{
	...
	printk("ti_sn_bridge_probe()+\n");

	if ( get_hardware_board_sku() != HW_BOARD_ND108T_8MQ_4G32G ) {
		printk("It's not HW_BOARD_ND108T_8MQ_4G32G SKU, Skip enabling MIPI-eDP bridge driver!!!\n");
		return -ENODEV;
	}
