...
static int sn65dsi83_brg_power_on(struct sn65dsi83_brg *brg)
{
    ...
    if (!IS_ERR(brg->gpio_panel_enable))
        gpiod_set_value(brg->gpio_panel_enable, 1);
    if (!IS_ERR(brg->gpio_backlight_enable))
        gpiod_set_value(brg->gpio_backlight_enable, 1);
...
{
    if(brg->lvds_panel_type == LVDS_PANEL_TIANMA_TM070JDHG30){
        if (!IS_ERR(brg->gpio_backlight_enable)){
            gpiod_set_value(brg->gpio_backlight_enable, 0);
            mdelay(100);
        }
    }
    if (!IS_ERR(brg->gpio_panel_enable)){
        gpiod_set_value(brg->gpio_panel_enable, 0);
    }
    /*
     * The EN pin must be held low for at least 10 ms
     * before being asserted high
     */
    mdelay(brg->power_off_time);
}
...
static void sn65dsi83_brg_stop_stream(struct sn65dsi83_brg *brg)
{
    ...
    if(brg->lvds_panel_type == LVDS_PANEL_AUO_M215HTN01){
        if (!IS_ERR(brg->gpio_backlight_enable)){
            gpiod_set_value(brg->gpio_backlight_enable, 0);
            mdelay(150);
        }
...
static int sn65dsi83_brg_configure(struct sn65dsi83_brg *brg)
{
    ...
    /* Voltage and pins */
    dev_info(&client->dev, "lvds_vod_swing=0x%x\n", brg->lvds_vod_swing);
    SN65DSI83_WRITE(SN65DSI83_LVDS_SIGN,brg->lvds_vod_swing);
    ...
    /* Seq 05: Initialize all CSR registers */
    /* Geometry */
