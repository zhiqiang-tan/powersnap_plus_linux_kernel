...
struct sn65dsi83 {
    ...
    bool enabled; //twice-init behavior. disable:0 enable:1
};
...
static void sn65dsi83_bridge_enable(struct drm_bridge *bridge)
{
    ...
    if (sn65dsi83->enabled != 0) //twice-init detect
        return;
    ...
    sn65dsi83->enabled = 1; //twice-init control
}
...
static void sn65dsi83_bridge_disable(struct drm_bridge *bridge)
{
    ...
    sn65dsi83->brg->funcs->stop_stream(sn65dsi83->brg);
    sn65dsi83->enabled = 0; //Initial the enabled flag for twice-init behavior.
}
    ...
    struct sn65dsi83 *sn65dsi83)
{
    ...
    sn65dsi83->brg->lvds_vod_swing = 0x0f;
    ...
    sn65dsi83->brg->gpio_panel_enable = devm_gpiod_get(dev, "enable-panel", GPIOD_OUT_HIGH);
    if (IS_ERR(sn65dsi83->brg->gpio_panel_enable)) {
        dev_err(dev, "failed to parse enable gpio");
        return PTR_ERR(sn65dsi83->brg->gpio_panel_enable);
    }

    sn65dsi83->brg->gpio_backlight_enable = devm_gpiod_get(dev, "enable-backlight", GPIOD_OUT_HIGH);
    if (!IS_ERR(sn65dsi83->brg->gpio_backlight_enable)) {
        msleep(200);
    } else
    ...
    const struct i2c_device_id *id)
{
	...
	if ( (get_hardware_board_sku() != HW_BOARD_ND108T_8MD_1G8G_LVDS)  &&
         (get_hardware_board_sku() != HW_BOARD_ND108T_8MD_1G8G_LVDS_ONLY_COM) &&
         (get_hardware_board_sku() != HW_BOARD_ND108T_8MD_2G16G_LVDS) &&
         (get_hardware_board_sku() != HW_BOARD_ND108T_8MQ_4G32G_LVDS) &&
         (get_hardware_board_sku() != HW_BOARD_ND108T_8MQ_4G32G_LVDS_ONLY)){
		dev_err(dev, "It's not 1G8G_LVDS, 1G8G_LVDS_SINGLE, 2G16G_LVDS, 8MD_4G32G_LVDS or 4G32G_LVDS_SINGLE SKU, Skip enabling MIPI-LVDS bridge driver!!!\n");
		return -ENODEV;
	}
...
static int sn65dsi83_remove(struct i2c_client *i2c)
{
    ...
    if (sn65dsi83->backlight)
        put_device(&sn65dsi83->backlight->dev);
    drm_bridge_remove(&sn65dsi83->bridge);

    return 0;
}

static void sn65dsi83_shutdown(struct i2c_client *i2c)
{
    struct sn65dsi83 *sn65dsi83 = i2c_get_clientdata(i2c);
    struct device *dev = &sn65dsi83->brg->client->dev;
    dev_dbg(dev, "%s\n",__func__);
