...
static void
nwl_dsi_bridge_post_disable(struct drm_bridge *bridge)
{
			...
			struct drm_display_mode *adjusted_mode)
{
	...
	/*
	 * This source code block was originally used by the nwl_dsi_bridge_atomic_check
	 * function, but on the v4.14.98 there is no support for such routine.
	 */
	if (!dsi->use_dcss && !dsi->pdata->use_dcnano_or_epdc) {
		/* At least LCDIF + NWL needs active high sync */
...
static void
nwl_dsi_bridge_pre_enable(struct drm_bridge *bridge)
{
	struct nwl_dsi *dsi = bridge_to_dsi(bridge);
	int ret;
...
#else
	/* XXX:
	 * Call the DSI bridge enable function explicitly due to some broken dependencies
	 * on the v4.14.98 Kernel when calling the drm_bridge_chain_enable function
	 */
...
static void
nwl_dsi_bridge_enable(struct drm_bridge *bridge)
{
	struct nwl_dsi *dsi = bridge_to_dsi(bridge);
	int ret;
...
{	struct nwl_dsi *dsi = bridge_to_dsi(bridge);
	...
	brdg = of_drm_find_bridge(remote);
	drm_panel_bridge_remove(brdg);
...
static const struct drm_bridge_funcs nwl_dsi_bridge_funcs = {
	...
	.post_disable   = nwl_dsi_bridge_post_disable,
