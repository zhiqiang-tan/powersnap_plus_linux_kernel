			    ...
			    struct serial_rs485 *rs485conf)
{
	...
	if (rs485conf->flags & SER_RS485_ENABLED) {
		...
		if (sport->have_rtscts && !(rs485conf->flags & SER_RS485_RTS_ON_SEND))
		...
		struct platform_device *pdev)
{
	...
	if (of_get_property(np, "fsl,rs485-enabled-at-boot-time", NULL) &&
		( (get_hardware_board_sku() == HW_BOARD_ND108T_8MQ_4G32G) || 
          (get_hardware_board_sku() == HW_BOARD_ND108T_8MQ_4G32G_LVDS) )&&
		get_serial_port_mode() == F81439_RS485_HALF_DUPLEX_1T_1R_HA_MODE) {
		pr_info("%s - RS485 is enabled.\n", dev_name(&pdev->dev));
